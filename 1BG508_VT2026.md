...
But content largely based on: <https://github.com/nikvuk92/1BG508>
...
In most of genomic analyses, we don't want any related individuals: they're basically giving us gene copies of the same thing. In order to identify related pairs of individuals, we usually use [KING](http://people.virginia.edu/~wc9c/KING/manual.html). Check out the manual and try and figure out how the software works! We can also go over it in the class.
...
# Get the full names of individuals to remove from the .fam file and save to sample_list.txt
rm sample_list.txt
for i in $(cat ID_list.txt); do grep $i ${DB}.fam; done | sort | uniq > sample_list.txt
